"use client";

import React, { useState, useEffect } from 'react';
import Header from '../components/Header';
import Footer from '../components/Footer';
import { supabase } from '@/lib/supabase';

export default function ReturnPolicyPage() {
  const [content, setContent] = useState("");
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    async function fetchPolicy() {
      try {
        const { data, error } = await supabase
          .from('store_settings')
          .select('return_policy')
          .eq('id', 'default')
          .single();

        if (error) {
          console.error("Error fetching return policy:", error);
        } else if (data && data.return_policy) {
          setContent(data.return_policy);
        }
      } catch (err) {
        console.error("Unexpected error:", err);
      } finally {
        setLoading(false);
      }
    }

    fetchPolicy();
  }, []);

  return (
    <div className="bg-white min-h-screen flex flex-col">
      <Header />
      
      <main className="flex-1 w-full max-w-4xl mx-auto px-6 py-16 text-gray-800">
        <h1 className="text-3xl md:text-4xl font-black mb-8 text-black border-b border-gray-200 pb-4">
          No Return or Refund Policy for Embroidery and Custom Apparel
        </h1>

        {loading ? (
          <div className="flex justify-center py-12">
            <div className="w-8 h-8 border-4 border-black border-t-transparent rounded-full animate-spin"></div>
          </div>
        ) : content ? (
          <div 
            className="prose prose-base max-w-none text-gray-800 font-medium leading-relaxed 
                       prose-h2:text-xl prose-h2:font-bold prose-h2:text-black prose-h2:mb-3 prose-h2:mt-8
                       prose-p:mb-4
                       prose-ul:list-disc prose-ul:pl-6 prose-ul:mt-2 prose-ul:space-y-1
                       prose-strong:text-black"
            // 🚀 AQUÍ ESTÁ LA MAGIA: Reemplaza los Non-Breaking Spaces por espacios normales
            dangerouslySetInnerHTML={{ __html: content.replace(/&nbsp;/g, ' ') }} 
          />
        ) : (
          <p className="text-gray-500">Return policy content is currently unavailable.</p>
        )}
      </main>

      <Footer />
    </div>
  );
}