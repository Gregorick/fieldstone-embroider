...
export async function POST(req: Request) {
  try {
    // 1. INICIALIZAMOS LAS HERRAMIENTAS ADENTRO DE LA FUNCIÓN PARA EVITAR CRASHES
    ...
    // 2. EXTRAEMOS LA FIRMA USANDO EL MÉTODO NATIVO DE LA REQUEST
    ...
    // 3. PROCESAMOS EL EVENTO DE PAGO EXITOSO
    if (event.type === 'checkout.session.completed') {
      ...
      if (orderId) {
        ...
        // 🚀 AQUÍ ESTÁ LA CORRECCIÓN: Se eliminó "status: 'paid'"
        ...
        if (updatedOrder) {
          ...
          try {
              ...
              html: `
                <div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; max-width: 650px; margin: 0 auto; border: 2px solid #10b981; border-radius: 12px; overflow: hidden; background-color: #ffffff;">
                  ...
                  <div style="padding: 24px;">
                    ...
                    <h3 style="color: #111827; border-bottom: 2px solid #e5e7eb; padding-bottom: 12px; font-size: 18px; font-weight: 900; text-transform: uppercase;">Products (${dbItems.length})</h3>
            ...
            console.log("✅ Correos de confirmación enviados exitosamente.");
             ...
             console.error("❌ Error ejecutando correos:", emailErr);
          }
        }
      }
    }
    ...
    console.error("❌ ERROR GLOBAL EN WEBHOOK:", globalError);
