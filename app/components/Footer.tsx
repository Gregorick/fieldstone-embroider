          ...
          <div>
            ...
            <ul className="space-y-4 text-gray-400 text-xs font-bold uppercase tracking-widest">
              {productCategories.map((category, index) => (
                <li key={index}><Link href={category.href} className="hover:text-blue-500 transition-colors">{category.name}</Link></li>
              ))}
            </ul>
          </div>
          ...
          <div>
            ...
            <form className="space-y-4" onSubmit={handleSubscribe}>
              ...
              <div className="flex items-start gap-3 pt-2">
                <input type="checkbox" id="terms" required checked={termsAccepted} onChange={e => setTermsAccepted(e.target.checked)} className="mt-1 accent-blue-600 cursor-pointer" />
                <label htmlFor="terms" className="text-[9px] text-gray-500 leading-tight cursor-pointer">I agree to the terms and conditions and the privacy policy</label>
              </div>
              {message && (
                <p className={`text-[10px] font-bold uppercase tracking-widest mt-2 ${message.type === 'success' ? 'text-green-500' : 'text-red-500'}`}>{message.text}</p>
              )}
            </form>
          </div>
        </div>
