...
// ✅ 2. DICCIONARIO: Conecta el nombre exacto de la marca con la Imagen
...
export default function FeaturedBrands() {
  ...
  useEffect(() => {
    async function fetchFeaturedBrands() {
      setLoading(true);
      try {
        // Consultar la configuración del administrador para ver si ocultó alguna marca
        ...
        // 🔥 LA SOLUCIÓN: Usamos las llaves del mapa (las 35 marcas) directamente sin escanear los productos.
        let availableBrands = Object.keys(brandImagesMap);

        // Filtramos por las marcas permitidas por el admin (si es que configuró alguna)
        if (allowedBrands.length > 0) {
          availableBrands = availableBrands.filter(b => allowedBrands.includes(b));
        }

        // Ordenamos: Las TOP primero, luego el resto en orden alfabético
